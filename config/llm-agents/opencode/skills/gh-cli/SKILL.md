---
name: gh-cli
description: Use the GitHub CLI (gh) to interact with GitHub - manage issues, pull requests, repositories, code search, Actions, projects, releases, gists, and other GitHub operations from the command line.
---

# GitHub CLI (gh)

Use the `gh` CLI to interact with GitHub from the command line. Prefer `gh` over the GitHub API or web browser for managing issues, pull requests, repositories, code search, Actions, and other GitHub operations.

> **Important:** The `gh` CLI should already be installed and authenticated. If a `gh` command fails due to missing installation or authentication, notify the user and do not attempt to install or configure `gh` yourself.
...
2. **JSON Parsing**: Use `--json` and `--jq` for structured data extraction
